...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [authMode, setAuthMode] = useState<AuthMode>(AUTH_MODE);
      ...
      try {
        if (AUTH_MODE === 'KEYCLOAK') {
          try {
            ...
            // Keycloak unreachable — fall back to local password auth
            if (mounted) setAuthMode('LOCAL');
  ...
  const login = async () => {
    if (authMode !== 'KEYCLOAK') {
      throw new Error('Use loginLocal in LOCAL mode');
    }
    const kc = getKeycloak();
  ...
  const register = async () => {
    if (authMode !== 'KEYCLOAK') {
      throw new Error('Use registerLocal in LOCAL mode');
    }
    const kc = getKeycloak();
  ...
  const registerLocal = async (email: string, password: string, fullName: string) => {
    const { token } = await api.authRegister(email, password, fullName);
    await setLocalToken(token);
  };
  ...
  const value = useMemo<AuthCtx>(
    () => ({ user, loading, authMode, login, register, loginLocal, registerLocal, logout, setLocalToken, clearLocalToken }),
